...
const SuccessfullLoader = ({successfullMessage, circle}) => {
  ...
  // Effect to update the message when successfullMessage prop changes
  useEffect(() => {
    ...
    <div className="relative">
      <div className="fixed top-0  left-0 right-0 bottom-0 flex justify-center items-center bg-black bg-opacity-80 z-50">
        <div className="content p-6 flex min-h-screen flex-col items-center justify-center rounded-lg ">
          <>
            <h4 className="text-[24px] lg:text-[36px] text-white font-bold text-center fixed top-[15rem] md:top-[16rem] font-karla mb-6 transition-all duration-300 ease-in-out">
              {message}
              <span
                className={`inline-block ${
                  ellipsisVisible ? 'animate-bounce' : 'opacity-0'
                }`}
              >
                ...
              </span>
